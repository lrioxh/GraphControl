...
class GCC_LoRA(nn.Module):
    ...
    def __init__(
        ...
        if degree_input:
            self.degree_embedding = lora.Embedding(
        ...
        self.lin_readout = nn.Sequential(
            ...
            lora.Linear(2 * node_hidden_dim, node_hidden_dim, r = lora_rank),
            nn.ReLU(),
            lora.Linear(node_hidden_dim, output_dim, r = lora_rank),
        )
        self.norm = norm
...
class SELayer(nn.Module):
    ...
    def __init__(self, in_channels, se_channels, lora_rank=16):
        ...
        self.encoder_decoder = nn.Sequential(
            lora.Linear(in_channels, se_channels, r = lora_rank),
            nn.ELU(),
            lora.Linear(se_channels, in_channels, r = lora_rank),
            nn.Sigmoid(),
        )
...
class ApplyNodeFunc(nn.Module):
    ...
    def __init__(self, mlp, use_selayer, lora_rank):
        super(ApplyNodeFunc, self).__init__()
        self.mlp = mlp
        self.bn = (
            SELayer(self.mlp.output_dim, int(np.sqrt(self.mlp.output_dim)), lora_rank)
            if use_selayer
...
class MLP(nn.Module):
    ...
    def __init__(self, num_layers, input_dim, hidden_dim, output_dim, use_selayer, lora_rank=16):
        ...
        if num_layers < 1:
            ...
            # Linear model
            self.linear = lora.Linear(input_dim, output_dim, r = lora_rank)
        else:
            # Multi-layer model
            ...
            self.linears.append(lora.Linear(input_dim, hidden_dim, r = lora_rank))
            for layer in range(num_layers - 2):
                self.linears.append(lora.Linear(hidden_dim, hidden_dim, r = lora_rank))
            self.linears.append(lora.Linear(hidden_dim, output_dim, r = lora_rank))

            for layer in range(num_layers - 1):
                self.batch_norms.append(
                    SELayer(hidden_dim, int(np.sqrt(hidden_dim)), lora_rank)
                    if use_selayer
...
class UnsupervisedGIN(nn.Module):
    ...
    def __init__(
        ...
        for layer in range(self.num_layers - 1):
            if layer == 0:
                mlp = MLP(
                    num_mlp_layers, input_dim, hidden_dim, hidden_dim, use_selayer, lora_rank
                )
            else:
                mlp = MLP(
                    num_mlp_layers, hidden_dim, hidden_dim, hidden_dim, use_selayer, lora_rank
                )

            self.ginlayers.append(
                GINConv(
                    ApplyNodeFunc(mlp, use_selayer, lora_rank),
                    0,
                    self.learn_eps,
                )
            )
        ...
        for layer in range(num_layers):
            if layer == 0:
                self.linears_prediction.append(
                    lora.Linear(input_dim, output_dim, r = lora_rank))
            else:
                self.linears_prediction.append(
                    lora.Linear(hidden_dim, output_dim, r = lora_rank))
